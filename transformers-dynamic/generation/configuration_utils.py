...
class GenerationConfig(PushToHubMixin):
    ...
    Arg:
        ...
        entropy_decoding (`str`, *optional*):
            针对部分 decoder-only 模型（目前主要是 `gpt-oss`）的基于中间层熵的动态解码策略。
            取值为 `\"trough\"` 时，每一步根据所有层 logits 的信息熵选取“熵谷”所在层进行解码；
            取值为 `\"random_after\"` 时，在“熵谷”层及其之后的层中按均匀分布随机选择一层进行解码。
            当为 `None`（默认）时，不启用该策略，仍使用末层 logits 进行解码。
        entropy_record_tokens (`bool`, *optional*, defaults to `False`):
            当为 `True` 时，在启用 `entropy_decoding` 的解码过程中记录每一步、每一层的预测 token id
            以及最终选择的 token id，并在 `generate(return_dict_in_generate=True)` 的输出中返回，便于统计与可视化分析。
    ...
    def __init__(self, **kwargs):
        ...
        # 【项目特有】基于中间层熵的动态解码：trough=熵谷选层，random_after=熵谷后随机选层；None=不启用
        self.entropy_decoding = kwargs.pop("entropy_decoding", None)
        # 是否记录每步、每层的 token id，用于熵解码分析
        self.entropy_record_tokens = kwargs.pop("entropy_record_tokens", False)
