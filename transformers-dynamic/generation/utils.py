...
from ..utils.entropy import (
    ...
    calculate_multi_layer_entropy_selection,
    _select_layers_from_entropies,
)
...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        ...
        entropy_per_layer_token_ids (`tuple(torch.LongTensor)`, *optional*, returned when `entropy_decoding` is enabled and `entropy_record_tokens=True`):
            Tuple of `torch.LongTensor` (one element for each generated token) of shape `(batch_size, num_layers)`.
            Each element contains the argmax token id for each layer at that generation step.
        entropy_selected_token_ids (`tuple(torch.LongTensor)`, *optional*, returned when `entropy_decoding` is enabled and `entropy_record_tokens=True`):
            Tuple of `torch.LongTensor` (one element for each generated token) of shape `(batch_size,)`.
            Each element contains the final selected token id at that generation step (after logits processing and sampling).
        entropy_selected_layer_indices (`tuple(torch.LongTensor)`, *optional*, returned when `entropy_decoding` is enabled):
            Tuple of `torch.LongTensor` (one element for each generated token) of shape `(batch_size,)`.
            Each element contains the layer index (0 to num_layers-1) that was actually selected for decoding at that generation step.
    """
    ...
    # 新增：基于内层熵解码时的辅助输出
    entropy_per_layer_token_ids: Optional[tuple[torch.LongTensor]] = None
    entropy_selected_token_ids: Optional[tuple[torch.LongTensor]] = None
    entropy_selected_layer_indices: Optional[tuple[torch.LongTensor]] = None
...
class GenerationMixin(ContinuousMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            # 【项目特有】若启用 entropy_decoding 且为 gpt_oss 模型，则走基于熵的动态层解码
            entropy_strategy = getattr(generation_config, "entropy_decoding", None)
            record_entropy_tokens = getattr(generation_config, "entropy_record_tokens", False)
            use_entropy_decoding = (
                entropy_strategy is not None
                and not self.config.is_encoder_decoder
                and getattr(self.config, "model_type", None) == "gpt_oss"
            )

            if use_entropy_decoding:
                result = self._entropy_decoding(
                    input_ids,
                    ...
                    synced_gpus=synced_gpus,
                    streamer=streamer,
                    record_entropy_tokens=record_entropy_tokens,
                    **model_kwargs,
                )
            else:
                result = self._sample(
    ...
    def _entropy_decoding(
        ...
        record_entropy_tokens: bool = False,
        **model_kwargs,
    ) -> Union[GenerateNonBeamOutput, torch.LongTensor]:
        """
        基于中间层熵的解码策略，仅用于 decoder-only 文本模型（当前主要是 gpt-oss）。

        - 当 `entropy_strategy == "trough"` 时：按熵谷层（信息熵最小的层）解码；
        - 当 `entropy_strategy == "random_after"` 时：在熵谷层及其之后的层中等概率随机选一层解码。
        """
        if self.config.is_encoder_decoder:
            raise ValueError("Entropy-based inner-layer decoding is only available for decoder-only models.")

        if not hasattr(self, "lm_head"):
            raise ValueError(
                "Entropy-based inner-layer decoding requires a CausalLM model with an `lm_head` attribute."
            )

        pad_token_id = generation_config._pad_token_tensor
        output_attentions = generation_config.output_attentions
        # 模型内部始终需要 hidden_states 来做分层熵计算
        ...
        entropy_per_layer_token_ids: Optional[tuple[torch.LongTensor]] = () if (
            return_dict_in_generate and record_entropy_tokens
        ) else None
        entropy_selected_token_ids: Optional[tuple[torch.LongTensor]] = () if (
            return_dict_in_generate and record_entropy_tokens
        ) else None
        # 记录每步实际选择的层索引（无论是否开启 record_entropy_tokens 都记录）
        entropy_selected_layer_indices: Optional[tuple[torch.LongTensor]] = () if return_dict_in_generate else None
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # 强制模型在内部返回 hidden_states 以供熵计算，同时保留用户的输出控制
            ...
            # 取得所有层在当前 step 的 hidden states（decoder-only: outputs.hidden_states）
            normalized_hidden_states = getattr(outputs, "normalized_hidden_states", None)
            all_layer_hidden_states = normalized_hidden_states if normalized_hidden_states is not None else outputs.hidden_states
            ...
            # tuple(len = num_layers + 1), [B, seq_len, H]
            # 跳过 embedding 输出，只保留各层输出
            per_layer_step_hiddens = [hs[:, -1, :] for hs in all_layer_hidden_states[1:]]  # L * [B, H]

            # 计算每层的 raw logits
            logits_per_layer: list[torch.Tensor] = []
            for hs in per_layer_step_hiddens:
                logits = self.lm_head(hs)  # [B, V]
                logits_per_layer.append(logits)

            L = len(logits_per_layer)
            device = input_ids.device

            # 对每一层的 logits 都应用 logits_processor，得到处理后的 scores
            scores_per_layer: list[torch.Tensor] = []
            for layer_logits in logits_per_layer:
                # 转换为 float32 并应用 logits_processor
            ...
            # 基于处理后的 scores 计算每层的熵
            entropies_per_layer: list[torch.Tensor] = []
            for layer_scores in scores_per_layer:
                entropy = calculate_information_entropy(layer_scores)  # [B]
                entropies_per_layer.append(entropy)

            # 基于熵谷选择层索引
            selected_idx_trough = _select_layers_from_entropies(
                entropies_per_layer,
            ...
            if entropy_strategy == "trough":
                ...
                rand = torch.rand_like(selected_idx_trough, dtype=torch.float, device=device)
                offsets = torch.floor(rand * counts.to(torch.float)).to(torch.long)
            ...
            # 记录每步实际选择的层索引
            if return_dict_in_generate:
                entropy_selected_layer_indices = entropy_selected_layer_indices + (final_idx,)

            # 从处理后的 scores 中选取最终层的 scores（已经过 logits_processor 处理）
            ...
            # 如需记录各层 argmax token 与最终选择层的 argmax token
            if return_dict_in_generate and record_entropy_tokens:
                # 记录每层处理后的 scores 的 argmax（而非 raw logits 的 argmax）
                per_layer_token_ids_step = torch.stack(
                    [layer_scores.argmax(dim=-1) for layer_scores in scores_per_layer], dim=1
                )  # [B, L]
                entropy_per_layer_token_ids = entropy_per_layer_token_ids + (per_layer_token_ids_step,)
                # 真正的采样 token（考虑 sampling）会在下方 token 采样后再记录
            ...
            if return_dict_in_generate:
                if output_scores:
                    scores += (next_token_scores,)
                if output_logits:
                    # 获取最终选择层的 raw logits（未经过 logits_processor 处理）
                    logits_stack = torch.stack(logits_per_layer, dim=1)  # [B, L, V]
                    batch_indices = torch.arange(logits_stack.size(0), device=device)
            ...
            # 记录“最终策略选择的 token_id”（即真正 decode 出来的 token，而非 raw argmax）
            if return_dict_in_generate and record_entropy_tokens:
                entropy_selected_token_ids = entropy_selected_token_ids + (next_tokens,)
