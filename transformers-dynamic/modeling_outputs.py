...
class BaseModelOutput(ModelOutput):
    ...
    Args:
        ...
        normalized_hidden_states (`tuple(torch.FloatTensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple mirroring `hidden_states`, where each tensor has been further normalized by the model's final layernorm
            (e.g. `self.norm` in decoder-only models). Useful when downstream algorithms expect layer-wise normalized
            representations.
...
class MoECausalLMOutputWithPast(ModelOutput):
    ...
    past_key_values: Optional[Cache] = None
    hidden_states: Optional[tuple[torch.FloatTensor, ...]] = None
    normalized_hidden_states: Optional[tuple[torch.FloatTensor, ...]] = None
    attentions: Optional[tuple[torch.FloatTensor, ...]] = None
...
class MoEModelOutput(ModelOutput):
    ...
    Args:
        ...
        hidden_states (`tuple(torch.FloatTensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            ...
            Hidden-states of the model at the output of each layer plus the optional initial embedding outputs.
        normalized_hidden_states (`tuple(torch.FloatTensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple mirroring `hidden_states`, where each tensor has been passed through the model's final normalization
            layer (when applicable). This is useful for algorithms that require normalized representations at every
            decoder layer.
        attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.
